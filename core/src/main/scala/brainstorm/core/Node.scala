...
* }}}
*
* @constructor Create a new node with `line` and `parent`. Adds itself as parents child.
  ...
  **/
  ...
  override def equals(obj: Any): Boolean =
    if (obj.isInstanceOf[Node]) {
      ...
      check.line == line && check.children.sameElements(children)
    } else {
      false
    }
}
